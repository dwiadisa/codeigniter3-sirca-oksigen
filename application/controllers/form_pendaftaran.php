...
class Form_pendaftaran extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        // load hak akses model
        // load model untuk kondisi formulir keadaan aktif
        $this->load->model('Aktivasi_formulir_model');
        // load model untuk kondisi formulir keadaan aktif 
    ...
    public function index()
    {
        ...
        // load model pada database untuk form ubah
        // query untuk status formulir aktif maupun nonaktif
        $form_aktif = $this->db->query("SELECT * FROM `setting` WHERE `nama_setting`='form_aktif'  LIMIT 1")->result();
        // query untuk status formulir aktif maupun nonaktif
        foreach ($form_aktif as $aktif) {
            $this->load->view('templates/header_sidebar', $data);
            // var_dump($aktif->value);
            $status = $aktif->value;
            //    pengondisian ketika formulir terbuka dan tertutup oleh admin
            if ($status == 1) {
                // echo "aktif";
                $this->load->view('partial/formulir_pendaftaran/formulir_pendaftaran', $data);
            } else {
                # code...
                $this->load->view('partial/formulir_pendaftaran/formulir_nonaktif');
                // echo "nonaktif";
            }
    ...
    public function simpan()
    {
        ...
        // query untuk status formulir aktif maupun nonaktif
        // jika aktif bisa disimpan jika tidak aktif maka diarahkan redirect ke 403
        $form_aktif = $this->db->query("SELECT * FROM `setting` WHERE `nama_setting`='form_aktif'  LIMIT 1")->result();
        // query untuk status formulir aktif maupun nonaktif
        foreach ($form_aktif as $aktif) {
            // var_dump($aktif->value);
